...
export default function Sidebar({
    ...
    return (
        <>
            ...
            <aside
                className={`fixed top-0 left-0 z-50 h-full w-72 bg-surface backdrop-blur-md border-r border-border-subtle transition-transform duration-300 lg:translate-x-0 lg:static lg:z-auto ${open ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                ...
                <nav className="flex flex-col gap-1.5 px-4">
                    {links.map((link) => (
                        <NavLink
                            ...
                            onClick={onClose}
                            className={({ isActive }) =>
                                `flex items-center gap-3.5 px-5 py-3 rounded-xl text-base transition-all duration-200 ${isActive
